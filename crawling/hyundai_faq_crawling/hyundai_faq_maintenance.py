...
# 3. 차량정비 탭 클릭 (필요x)
hcar_m_btn = driver.find_element(By.XPATH, '//*[@id="app"]/div[3]/section/div[2]/div/div[2]/section/div/div[1]/div[1]/ul/li[2]/button')
hcar_m_btn.click()
time.sleep(2)
...
# 4. 페이지 이동 및 데이터 수집 (차량구매/차량정비 4페이지)
def pages():
    for i in range(3):
...
df.to_csv('hyundai_maintenance_purchase.csv', index=False, encoding='utf-8-sig')
print('@@@CSV 저장 완료!@@@')

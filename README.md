...
- 이재은 : ERD 설계 과정에서 여러 차례 시행착오를 겪으며 데이터 모델링 방법을 명확히 이해하게 되었다. 현대 FAQ 크롤링을 진행하면서 페이지 넘어가는 버튼이 작동하지 않고 텍스트가 제대로 추출되지 않는 문제를 경험했지만, 이를 해결하며 문제 해결 능력이 크게 향상되었다. 이런 과정을 통해 실무에서 발생하는 예기치 않은 오류들을 직접 다뤄보면서 기술적 성장과 실전 감각을 키울 수 있었다.
